...
//	int ratePositional(int material);
	...
	{
	    int rate=0, i=0;
	    int tempPositionKing = chess.kingPositionL;
	    for(i=0; i<64; i++)
	    {
	        switch(chess.chessBoard[i/8][i%8])
	        {
	            case 'P':   chess.kingPositionL=i;
	                        if(!chess.kingSafe()){rate-=64;}
	                        break;
	            case 'R':   chess.kingPositionL=i;
	                        if(!chess.kingSafe()){rate-=500;}
	                        break;
	            case 'K':   chess.kingPositionL=i;
	                        if(!chess.kingSafe()){rate-=300;}
	                        break;
	            case 'B':   chess.kingPositionL=i;
	                        if(!chess.kingSafe()){rate-=300;}
	                        break;
	            case 'Q':   chess.kingPositionL=i;
	                        if(!chess.kingSafe()){rate-=900;}
	                        break;
	        }
	    }
	    chess.kingPositionL=tempPositionKing;
...
//	                            rate+=StringLength(posibleA(kingPositionC))*10;
	                            rate+=chess.posibleA(chess.kingPositionL).length()*10;
	                        }else
	                        {
	                            rate+=chess.kingEndBoard[i/8][i%8];
//	                            rate+=StringLength(posibleA(kingPositionC))*30;
	                            rate+=chess.posibleA(chess.kingPositionL).length()*30;
	                        }
	                        break;
	        }
	    }

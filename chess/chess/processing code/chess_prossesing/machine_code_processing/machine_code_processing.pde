...
String alphaBeta(int depth, int beta, int alpha, String move, int player)
{
    ...
    for(int i=0; i<StringLength(list); i+=5)
    {
        ...
        String temp2 = "";
        int j = 0;
        while(returnString.charAt(j+5) != '\0'){temp2 = temp2.substring(0, j)+returnString.charAt(j+5)+temp2.substring(j+1); j++;}
        int value = Integer.parseInt(temp2);
        flipBoard();
...
/*String sortMoves(String list)
{
    int score[] = new int[StringLength(list)/5];
    for(int i=0; i<StringLength(list); i+=5)
    {
        makeMove(list.substring(i, i+5));
    ...
    {//first few moves only
        ...
        newListB = newListB.substring(0, maxLocation*5) + newListB.substring(maxLocation*5+5, StringLength(newListB)-1);
    }
...
void draw()
{
  ...
  if ( myPort.available() > 0) {  // If data is available,
    move = myPort.readString();         // read it and store it in val
  }
  myPort.clear();
  makeMove(move);        // the human move is made
  findKing();
  if(kingPositionC == 64 || kingPositionL == 64){stop();};    // if either of the king dies the program stops
